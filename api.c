...
static void xbee_logf(const char *logformat, const char *function, char *format, ...) {
  ...
  fprintf(log,logformat,function,buf);
}
...
static int xbee_sendATdelay(int preDelay, int postDelay, char *command, char *retBuf, int retBuflen) {
  ...
  memset(&to, 0, sizeof(to));
  /* select on the xbee fd... wait at most 200ms for the response */
  to.tv_usec = 200000;
...
static int xbee_startAPI(void) {
  ...
  if (xbee_sendATdelay(xbee.cmdTime, xbee.cmdTime, buf, buf, sizeof(buf))) {
...
static int xbee_listen(t_info *info) {
  ...
  while (xbee.listenrun) {
    ...
    if (t == 0x8A) {
      ...
      if (xbee.log) {
        xbee_logc("Packet type: %d-bit RX I/O Data (0x%02X)\n",((t == 0x82)?64:16),t);
